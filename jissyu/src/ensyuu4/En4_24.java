...
public class En4_24 {
	//プログラムの説明のための定数
	private static final String PRIMENUMBER_PROGRAM_MESSAGE = "入力された値が素数であるか判定をします";
	//入力を促す文のための定数
	private static final String INPUT_NUMBER_MESSAGE = "素数判定をする数字を入力してください：";
	//素数であるという判定文のための定数
	private static final String IS_PRIMENUMBER_MESSAGE = "それは素数です。";
	//素数ではないという判定文のための定数
	private static final String NOT_PRIMENUMBER_MESSAGE = "それは素数ではありません。";
	...
	public static void main(String[] args) {
		...
		//プログラムの説明を出力する
		System.out.println(PRIMENUMBER_PROGRAM_MESSAGE);
		...
		for(int numberToDivide = 2; numberToDivide <= primeNumber; numberToDivide++){
			...
			if(numberToDivide == primeNumber){
				...
				System.out.println(IS_PRIMENUMBER_MESSAGE);
			//divideLeftNumber（剰余の値）が0と等しい場合、素数ではないという判定文を出力する
			}else if(divideLeftNumber == 0){
				//素数ではないという判定文を出力する
				System.out.println(NOT_PRIMENUMBER_MESSAGE);
