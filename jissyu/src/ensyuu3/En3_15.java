...
public class En3_15 {
	...
	public static void main(String[] args) {
		...
		//降順にソートするため、二値の入れ替えを行う
		//二値が等しかった場合、ソート結果文を出力しないためのラベルaのブロックでif文を囲む
		a : {
			//もし二値の値が等しい場合
			if (max == min){
				//二つの値は等しいという判定分を出力
				System.out.println("二つの値は等しいです");
				//ソート結果文を出力しないよう、ラベルaのついたブロックを抜ける
				break a;

			//もし、maxの値がminより小さかった場合,二値の入れ替えを行う
			}if(max < min){
				//予備変数subにmaxの値を代入する
				sub = max;
				//maxにminの値を代入する
			...
			//降順にソートした結果を出力する。もし二値が等しかった場合、breakによってこの文は実行されない。
